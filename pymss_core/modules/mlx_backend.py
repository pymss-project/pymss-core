...
def pad_last(x, left, right, mode="constant", value=0.0, extend=False):
    ...
    if mode != "reflect":
        raise TypeError(f"unsupported padding mode for MLX full backend: {mode!r}")
...
def batch_norm(module, x, dtype):  # eval mode only (inference package); 1d/2d/any-ndim
    ...
    if module.training:
        raise TypeError("MLX BatchNorm supports eval mode only")
...
def lstm(rnn, x, dtype):
    ...
    if rnn.num_layers != 1 or not rnn.batch_first:
        raise TypeError("MLX RNN supports one-layer batch_first RNNs only")
...
def rnn_forward(rnn, x, dtype):
    if isinstance(rnn, torch.nn.LSTM):
        return lstm(rnn, x, dtype)
    if isinstance(rnn, torch.nn.GRU):
        return gru(rnn, x, dtype)
