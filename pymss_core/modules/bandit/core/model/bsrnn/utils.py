...
def band_widths_from_specs(band_specs): return [e - i for i, e in band_specs]

def check_nonzero_bandwidth(band_specs):
    if any(fend - fstart <= 0 for fstart, fend in band_specs): raise ValueError("Bands cannot be zero-width")

def check_no_overlap(band_specs):
    if any(fstart <= -1 for fstart, _ in band_specs): raise ValueError("Bands cannot overlap")

def check_no_gap(band_specs):
    assert band_specs[0][0] == 0
    prev = -1
    for s, e in band_specs:
...
class PerceptualBandsplitSpecification(BandsplitSpecification):
    def __init__(self, nfft, fs, fbank_fn, n_bands, f_min=0.0, f_max=None):
        ...
        self.filterbank = fbank_fn(n_bands, fs, f_min, fs / 2 if f_max is None else f_max, self.max_index)
        normalized_fb = self.filterbank / self.filterbank.sum(dim=0, keepdim=True)  # (n_bands, n_freqs)
    ...
    def get_band_specs(self): return self.band_specs
    def get_freq_weights(self): return self.freq_weights
...
def bark_filterbank(n_bands, fs, f_min, f_max, n_freqs):
    nfft = 2 * (n_freqs - 1)
    centers = np.linspace(hz_to_bark(f_min), hz_to_bark(f_max or fs / 2), n_bands)
...
MelBandsplitSpecification = _perceptual(mel_filterbank)
BarkBandsplitSpecification = _perceptual(bark_filterbank)
TriangularBarkBandsplitSpecification = _perceptual(triangular_bark_filterbank)
MiniBarkBandsplitSpecification = _perceptual(minibark_filterbank)
EquivalentRectangularBandsplitSpecification = _perceptual(erb_filterbank)