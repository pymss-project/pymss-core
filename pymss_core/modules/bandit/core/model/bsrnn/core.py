...
class MultiMaskBandSplitCoreBase(nn.Module):
    ...
    def forward(self, x, cond=None, compute_residual=True):
        ...
        return {"spectrogram": {stem: self.mask(x, mask_estimator(q, cond=cond)).reshape(batch, in_chan, n_freq, n_time)
...
class MultiSourceMultiMaskBandSplitCoreRNN(MultiMaskBandSplitCoreBase):
    def __init__(self, in_channel, stems, band_specs, require_no_overlap=False, require_no_gap=True,
        ...
        super().__init__()
        self.instantiate_bandsplit(in_channel=in_channel, band_specs=band_specs, require_no_overlap=require_no_gap,
