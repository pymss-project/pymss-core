...
server 以单模型方式启动，一个进程只加载一个 `MSSeparator`，可通过多个 served model name 暴露同一个模型。
...
| `GET` | `/v1/models/{model}` | 是 | 返回指定 served model 的元数据 |
...
`pymss.instruments` 是 `/v1/audio/separations` 的 `stems` 可选值。它来自已加载模型的 `separator.config.training.instruments`，不是从 catalog 的 `target_stem` 推导。

## GET /v1/models/{model}

`model` 必须匹配当前进程的任一 served model name。找不到时返回 `404 model_not_found`。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `model` | string | 是 | 当前 server 暴露的 served model name |
...
```http
POST /v1/audio/separations?model=bs_roformer_voc_hyperacev2&format=pcm_f32le&sample_rate=44100&channels=2&stems=vocals&response_format=zip&output_audio_format=wav
