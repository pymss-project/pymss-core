...
class DualPathRNN(nn.Module):
    def __init__(self, d_model, expand, bidirectional=True):
        ...
        self.lstm_layers = nn.ModuleList([LSTM(d_model, hidden_size, num_layers=1, bidirectional=bidirectional, batch_first=True) for _ in range(2)])
    ...
    def forward(self, x):
        ...
        x = self.linear_layers[0](
            self.lstm_layers[0](self.norm_layers[0](x).transpose(1, 3).contiguous().view(B * T, F, C))[0]
        ).view(B, T, F, C).transpose(1, 3) + x

        x = self.linear_layers[1](
            self.lstm_layers[1](self.norm_layers[1](x).transpose(1, 2).contiguous().view(B * F, C, T).transpose(1, 2))[0]
        ).transpose(1, 2).contiguous().view(B, F, C, T).transpose(1, 2) + x
