...
class AdaptiveHyperedgeGeneration(nn.Module):
    ...
    def forward(self, x):
        b, n, c = x.shape
        pooled = x.permute(0, 2, 1)
        f_ctx = torch.cat((
            F.adaptive_avg_pool1d(pooled, 1).squeeze(-1),
            F.adaptive_max_pool1d(pooled, 1).squeeze(-1),
        ), dim=1)

        proto = self.global_proto.unsqueeze(0) + self.context_mapper(f_ctx).view(b, self.num_hyperedges, c)
        ...
        return F.softmax(sim.mean(dim=1).permute(0, 2, 1), dim=-1)
...
class HypergraphConvolution(nn.Module):
    ...
    def forward(self, x, a):
        f_m = self.act(self.W_e(torch.bmm(a, x)))
        return x + self.act(self.W_v(torch.bmm(a.transpose(1, 2), f_m)))
...
class AdaptiveHypergraphComputation(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.hypergraph_conv(x_flat, a).permute(0, 2, 1).view(b, -1, h, w)
...
class HyperACE(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]) -> torch.Tensor:
        ...
        x_h = self.high_order_fuse(torch.cat([m(x_h) for m in self.high_order_branch], dim=1))
        x_l = self.low_order_branch(x_l)
        return self.final_fuse(torch.cat((x_h, x_l, x_s), dim=1))
...
class Decoder(nn.Module):
    ...
    def forward(self, enc_feats: List[torch.Tensor], h_ace: torch.Tensor):
        ...
        d5 = self.fusion_d5(self.skip_p5(p5), self.h_to_d5(F.interpolate(h_ace, size=p5.shape[2:], mode='bilinear')))
        d4 = self.up_d5(F.interpolate(d5, size=p4.shape[2:], mode='bilinear')) + self.skip_p4(p4)
        d4 = self.fusion_d4(d4, self.h_to_d4(F.interpolate(h_ace, size=d4.shape[2:], mode='bilinear')))
        d3 = self.up_d4(F.interpolate(d4, size=p3.shape[2:], mode='bilinear')) + self.skip_p3(p3)
        d3 = self.fusion_d3(d3, self.h_to_d3(F.interpolate(h_ace, size=d3.shape[2:], mode='bilinear')))
        d2 = self.up_d3(F.interpolate(d3, size=p2.shape[2:], mode='bilinear')) + self.skip_p2(p2)
        d2 = self.fusion_d2(d2, self.h_to_d2(F.interpolate(h_ace, size=d2.shape[2:], mode='bilinear')))
...
class FreqPixelShuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.out_conv(x.view(b, out_c, h, w * self.scale))
...
class SegmModel(nn.Module):
    ...
    def forward(self, x):
        ...
        dec_feat = F.interpolate(dec_feat, size=(h, dec_feat.shape[-1]), mode='bilinear', align_corners=False)
